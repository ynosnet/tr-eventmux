...
class M3uParserTests(unittest.TestCase):
    ...
    def test_kodiprop_clearkey_license_is_used_for_plain_mpd_url(self):
        text = "\n".join(
            [
                ...
                '#KODIPROP:inputstream.adaptive.drm_legacy=org.w3.clearkey|http://license.invalid/api/license/dzn/abc123',
                'http://media.invalid/api/dzn/live/abc123.mpd',
            ]
        )
        ...
        self.assertEqual(events[0].url, "http://media.invalid/api/dzn/live/abc123.mpd")
        self.assertEqual(
            events[0].license_url,
            "http://license.invalid/api/license/dzn/abc123",
        )
        self.assertEqual(events[0].stream_type, "pipe_drm")

    def test_amazon_kodiprop_clearkey_license_is_used(self):
        text = "\n".join(
            [
                '#EXTM3U',
                '#EXTINF:0001 tvg-id="amz123" group-title="Events", [26/06/20 18:05] ZDFsportstudio live-FIFA WM 2026',
                '#KODIPROP:contentlookup=False',
                '#KODIPROP:mimetype=application/dash+xml',
                '#KODIPROP:inputstream=inputstream.adaptive',
                '#KODIPROP:inputstream.adaptive.drm_legacy=org.w3.clearkey|http://license.invalid/api/license/amz/amz123',
                'http://media.invalid/api/amz/live/amz123.mpd',
            ]
        )

        events = app.parse_m3u(
            text,
            self.config(key="amazon", event_group_filter="Events", date_format="auto"),
        )

        self.assertEqual(len(events), 1)
        self.assertEqual(events[0].url, "http://media.invalid/api/amz/live/amz123.mpd")
        self.assertEqual(
            events[0].license_url,
            "http://license.invalid/api/license/amz/amz123",
        )
        self.assertEqual(events[0].stream_type, "pipe_drm")
    ...
    def test_live_marker_event_is_parsed_as_current_event(self):
        ...
        self.assertEqual(events[0].stable_key, "dazn|tvg:live123")
        self.assertEqual(events[0].license_url, "http://license.invalid/live123")
        self.assertEqual(events[0].stream_type, "pipe_drm")

    def test_parenthesized_rtl_live_countdown_sets_future_start(self):
        ...
        text = "\n".join(
            [
                ...
                '#KODIPROP:mimetype=application/dash+xml',
                '#KODIPROP:inputstream=inputstream.adaptive',
                '#KODIPROP:inputstream.adaptive.drm_legacy=org.w3.clearkey|http://license.invalid/api/license/rtl/event54',
                'http://media.invalid/api/rtl/live/event54.mpd',
            ]
        )
        ...
        self.assertEqual(events[0].stable_key, "rtl|tvg:event54")
        self.assertEqual(
            events[0].license_url,
            "http://license.invalid/api/license/rtl/event54",
        )
        self.assertEqual(events[0].stream_type, "pipe_drm")

    def test_rtl_live_countdown_keeps_first_calculated_start(self):
        config = self.config(
            key="rtl",
            event_group_filter="Events",
            date_format="auto",
    ...
    def test_rtl_live_countdown_restarts_when_title_changes(self):
        config = self.config(
            key="rtl",
            event_group_filter="Events",
            date_format="auto",
    ...
    def test_rtl_countdown_keeps_dated_start_after_event_has_started(self):
        config = self.config(
            key="rtl",
            event_group_filter="Events",
            date_format="auto",
    ...
    def test_tvg_id_keeps_stable_key_when_event_changes_to_live_marker(self):
        ...
        self.assertEqual(live.stable_key, "dazn|tvg:same123")
    ...
    def test_event_and_explicit_vod_entries_are_not_live_channels(self):
        text = "\n".join(
            [
                ...
                '#EXTINF:-1 tr-vod="0" tvg-id="rtl" group-title="Deutschland",RTL HD',
                "http://media.invalid/rtl.mpd",
            ]
        )

        channels = app.parse_live_channels(text, self.config())

        self.assertEqual([channel.name for channel in channels], ["RTL HD"])
        ...
        cfg["sources"] = [
            {
                "key": "amazon",
                "name": "Amazon",
                "source_m3u": "http://example.invalid/amazon.m3u",
                "slots": 1,
                "event_group_filter": "Events",
            }
        ]
        old_event = {
            "stable_key": "amazon|tvg:event1",
            "start": "2026-06-22T18:00:00+02:00",
            ...
            "sources": [
                {
                    ...
                    "dropped_events": [],
                    "slot_memory": {"amazon|tvg:event1": 1},
                    "slots": [
                        {
                            "id": 1,
                            "source_key": "amazon",
...
class ScheduleTests(unittest.TestCase):
        ...
        cfg.update(
            {
                ...
                "channel_name_template": "DAZN Event {slot}",
                "channel_id_template": "dazn.event{slot}",
                "group_title": "DAZN Events",
            }
        )
        cfg.update(overrides)
        return cfg
    ...
    def test_persisted_slot_memory_survives_reordered_live_events(self):
        ...
        events = [
            self.event("dazn|tvg:motor", "motor", "Alpha Motorland", start),
            self.event("dazn|tvg:prague", "prague", "Zulu Prague", start),
        ]
        previous = {"slot_memory": {"dazn|tvg:motor": 2}}
    ...
    def test_slot_memory_is_stored_with_scheduled_events(self):
        zone = ZoneInfo("Europe/Berlin")
        start = datetime.now(zone) + timedelta(minutes=5)
        events = [self.event("dazn|tvg:motor", "motor", "Alpha Motorland", start)]

        slots, _, _ = app.build_schedule(events, self.config())
        memory = app.slot_memory_from_slots(slots)

        self.assertEqual(memory["dazn|tvg:motor"], 1)
...
class PlaybackSelectionTests(unittest.TestCase):
                ...
                "sources": [
                    {
                        "key": "rtl",
                        "name": "RTL",
                        "source_m3u": "http://example.invalid/rtl.m3u",
                        "slots": 1,
                        "channel_number_start": 921,
                        "channel_name_template": "RTL Event {slot}",
                        "channel_id_template": "rtl.event{slot}",
    ...
    def state(self, start):
        event = {
            ...
            "title": "NFL Network",
            "url": "http://media.invalid/rtl/event54.mpd",
            "source_key": "rtl",
        }
        return {
            "slots": [
...
class ReplaySelectionTests(unittest.TestCase):
    def test_nominally_finished_event_is_not_replayed_while_stream_is_extended(self):
        ...
        state = {
            ...
            "slots": [
                {"id": 1, "source_key": "rtl", "events": [scheduled]},
                {"id": 2, "source_key": "rtl", "events": []},
            ],
        }

        replay = app.replay_event_for_slot(2, state, cfg, "rtl", now=now)
...
class OutputTests(unittest.TestCase):
    def state_and_config(self, event_overrides=None):
        ...
        slot = {
            ...
            "source_key": "dazn",
            "source_name": "DAZN",
            "channel_id": "dazn.event1",
            "name": "DAZN Event 1",
            "number": 911,
            "group_title": "DAZN Events",
            "stream_path": "/slot/dazn/1.ts",
            "events": [event],
        }
        state = {
    ...
    def test_xmltv_channel_includes_event_slot_logo(self):
        state, cfg = self.state_and_config()
        state["slots"][0]["logo_path"] = "/slot/dazn/1/logo"

        text = app.render_xmltv(state, cfg)

        self.assertIn(
            '<icon src="http://127.0.0.1:8787/slot/dazn/1/logo" />',
            text,
        )
    ...
    def test_status_page_links_current_streams(self):
        ...
        self.assertIn('href="/slot/dazn/1.m3u"', text)
        self.assertIn('data-stream-url="/slot/dazn/1.ts"', text)
        self.assertIn('href="/slot/dazn/1/ffmpeg"', text)
        self.assertIn('>Slovak Darts Open</a>', text)
        self.assertIn('>DAZN Event 1 (911)</a>', text)
    ...
    def test_slot_playlist_points_to_transport_stream(self):
        response = self.with_state(lambda: app.slot_source_playlist("dazn", 1))
        text = response.body.decode("utf-8")

        self.assertIn("#EXTM3U", text)
        self.assertIn("http://127.0.0.1:8787/slot/dazn/1.ts", text)
    ...
    def test_slot_playlist_includes_event_logo(self):
        ...
        try:
            app.load_config = lambda: cfg
            app.get_state = lambda: state
            response = app.slot_source_playlist("dazn", 1)
        finally:
            app.load_config = old_load_config
            app.get_state = old_get_state
        ...
        text = response.body.decode("utf-8")
        self.assertIn(
            'tvg-logo="http://127.0.0.1:8787/slot/dazn/1/logo"',
            text,
        )

    def test_event_slot_logo_endpoint_returns_transparent_png(self):
        response = self.with_state(lambda: app.source_slot_logo("dazn", 1))
    ...
    def test_all_configured_event_slot_logos_exist(self):
        expected = {
            *(f"event-{number}.png" for number in range(901, 907)),
            *(f"event-{number}.png" for number in range(911, 921)),
            *(f"event-{number}.png" for number in range(921, 926)),
        }
...
class LiveTvOutputTests(unittest.TestCase):
    ...
    def test_live_page_shows_channel_without_exposing_stream_url(self):
        ...
        self.assertIn("/live/rtl/abc123.m3u", text)
        self.assertIn('data-stream-url="/live/rtl/abc123.ts"', text)
        self.assertIn('/live/rtl/abc123/logo"', text)
        self.assertIn('id="theme-toggle"', text)
        self.assertIn('id="player-mode-toggle"', text)
        self.assertIn('id="web-player-modal"', text)
    ...
    def test_live_playlist_points_to_local_transport_stream(self):
        ...
        self.assertIn("http://127.0.0.1:8787/live/rtl/abc123.ts", text)
        self.assertIn(
            'tvg-logo="http://127.0.0.1:8787/live/rtl/abc123/logo"', text
        )
        self.assertNotIn("http://media.invalid/zdf.mpd", text)
        self.assertNotIn("https://img.invalid/zdf.png", text)

    def test_single_channel_playlist_points_to_local_transport_stream(self):
        response = self.with_state(lambda: app.live_channel_playlist("rtl", "abc123"))
        text = response.body.decode("utf-8")
    ...
    def test_logo_proxy_returns_supported_upstream_image(self):
        ...
        try:
            ...
            response = self.with_state(
                lambda: app.live_channel_logo("rtl", "abc123")
            )
        finally:
            app.requests.get = old_get
