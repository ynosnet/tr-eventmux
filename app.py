...
COMPACT_COUNTDOWN_RE = re.compile(
    r"^\[?\s*"
    r"(?:(?P<days>\d+)\s*T)?"
    r"(?:(?P<hours>\d+)\s*H)?"
    r"(?:(?P<minutes>\d+)\s*M)?"
...
GERMAN_DATETIME_HINT_RE = re.compile(
    r"(?:^|[\s•])"
...
DEFAULTS: dict[str, Any] = {
    ...
    "xmltv_socket_enabled": False,
    "xmltv_socket_path": "",
    "xmltv_socket_timeout_seconds": 10,
}
...
def normalized_sources(cfg: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        source.update(
            {
                ...
                "logo_key": safe_source_key(cfg.get("logo_key") or "default"),
                "_legacy_route": True,
            }
        )
    ...
    for index, item in enumerate(configured, start=1):
        ...
        source["logo_key"] = safe_source_key(item.get("logo_key") or f"provider{index}")
...
def german_datetime_hint(text: str, zone: ZoneInfo) -> Optional[tuple[datetime, str]]:
    match = GERMAN_DATETIME_HINT_RE.search(text)
    if not match:
        return None
    year = int(match["year"])
    if year < 100:
    ...
    try:
        ...
        LOG.warning("Ungültiger Event-Datumshinweis ignoriert: %s", text)
        return None
    cleaned = clean_event_title(f"{text[:match.start()]} {text[match.end():]}")
    return start, cleaned


def parse_german_datetime_hint(text: str, zone: ZoneInfo) -> Optional[datetime]:
    parsed = german_datetime_hint(text, zone)
    if not parsed:
        return None
    return parsed[0]
...
def parse_event_name(
    ...
    compact_countdown_match = (
        COMPACT_COUNTDOWN_RE.search(raw_name) if live_countdown_enabled else None
    )
    if compact_countdown_match and (
        compact_countdown_match["days"] is not None
        or compact_countdown_match["hours"] is not None
        or compact_countdown_match["minutes"] is not None
    ):
        remainder = compact_countdown_match["name"].strip()
        reference = reference_time or datetime.now(zone)
        datetime_hint = german_datetime_hint(remainder, zone)
        if datetime_hint:
            start, remainder = datetime_hint
        else:
            start = reference.replace(second=0, microsecond=0) + timedelta(
                days=int(compact_countdown_match["days"] or 0),
                hours=int(compact_countdown_match["hours"] or 0),
                minutes=int(compact_countdown_match["minutes"] or 0),
            )
        title, desc = split_event_title(remainder, split_mode)
        return start, title, desc

    datetime_hint = german_datetime_hint(raw_name, zone) if live_countdown_enabled else None
    if datetime_hint:
        start, remainder = datetime_hint
        title, desc = split_event_title(remainder, split_mode)
        return start, title, desc
    ...
    if match and date_format in {"auto", "ymd", "yy/mm/dd"}:
        ...
        if date_format in {"dm", "dd/mm"}:
            ...
            # Provider format used by some playlists: [MM/DD HH:MM].
            month, day = first, second
...
def has_live_countdown(raw_name: Any) -> bool:
    ...
    live_match = LIVE_RE.search(raw_text)
    if live_match and LIVE_COUNTDOWN_RE.fullmatch(live_match["name"].strip()):
        return True
    compact_match = COMPACT_COUNTDOWN_RE.search(raw_text)
    return bool(
        compact_match
        and (
            compact_match["days"] is not None
            or compact_match["hours"] is not None
            or compact_match["minutes"] is not None
        )
    )
...
def slot_logo_file(cfg: dict[str, Any], slot_id: int) -> Path:
    logo_key = safe_source_key(cfg.get("logo_key") or cfg.get("key", "default"))
    slot_logo = EVENT_LOGO_DIR / f"event-{logo_key}-slot{slot_id}.png"
    if slot_logo.is_file():
        return slot_logo
    return EVENT_LOGO_DIR / f"event-{logo_key}.png"
...
def slot_shells(cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for slot_id in range(1, count + 1):
        ...
        logo_file = slot_logo_file(cfg, slot_id)
        slot = {
            "id": slot_id,
            "source_key": source_key,
            "source_name": source_name,
...
def schedule_sort_key(event: SourceEvent, now: datetime) -> tuple[Any, ...]:
...
def build_schedule(
    ...
    for event in sorted(candidates, key=lambda item: schedule_sort_key(item, now)):
        preferred_id = next(
...
def slot_playback_badge(
    ...
    if normalized_mode == "preview":
        return ("Vorschau", "preview", "Stream ist vor dem offiziellen Start verfügbar")
    if normalized_mode == "replay":
        return ("Replay", "replay", "Altes Event wird auf einem freien Slot wiedergegeben")
    if normalized_mode == "live":
        source_cfg = source_config_for_key(cfg, source_key) or cfg
        try:
            start = datetime.fromisoformat(str(event["start"])).astimezone(now.tzinfo)
            stop = epg_stop(start, event.get("stop"), source_cfg, now.tzinfo)
        except (KeyError, TypeError, ValueError):
        ...
        if stop <= now:
            return (
                "Weiter verfügbar",
                "available",
                "EPG-Zeitfenster ist beendet, der Stream ist aber noch gelistet",
            )
        return ("Live", "live", "Event liegt im aktuellen EPG-Zeitfenster")
    return ("Aktiv", "active", "Aktiver Slot")


def slot_badge_html(label: str, css_class: str, title: str) -> str:
    return (
        f'<span class="slot-badge {html.escape(css_class, quote=True)}" '
...
def index() -> HTMLResponse:
    ...
    rows = []
    now = datetime.fromisoformat(str(status.get("now")))
    for slot in status["slots"]:
        current = slot.get("current_event")
        next_event = slot.get("next_event")
        current_text = "–"
        if current:
            badge_label, badge_class, badge_title = slot_playback_badge(
                current,
                slot.get("playback_mode"),
                cfg,
                str(slot.get("source_key", "default")),
                now,
            )
            current_text = str(current["title"])
            labelled_title = f"{badge_label}: {current_text}"
            current_html = (
                slot_badge_html(badge_label, badge_class, badge_title)
                + " "
                + player_link(
                    f'{slot["stream_path"][:-3]}.m3u',
                    slot["stream_path"],
                    current_text,
                    f'{slot["name"]} · {labelled_title}',
...
<head>
  ...
  <style>
    ...
    .slot-badge {{ display: inline-block; vertical-align: .08em; margin-right: .35rem; border: 1px solid var(--border); border-radius: .35rem; padding: .1rem .35rem; font-size: .74rem; font-weight: 800; line-height: 1.25; text-transform: uppercase; letter-spacing: 0; }}
    .slot-badge.live {{ border-color: var(--ok); color: var(--ok); background: var(--current); }}
    .slot-badge.available {{ border-color: var(--muted); color: var(--muted); background: var(--surface); }}
    .slot-badge.replay {{ border-color: var(--link); color: var(--link); background: var(--surface); }}
    .slot-badge.preview {{ border-color: var(--link); color: var(--link); background: var(--surface-strong); }}
...
def slot_logo_response(source_key: str, slot_id: int) -> Response:
    ...
    if source_cfg is None or not 1 <= slot_id <= int(source_cfg["slots"]):
        raise HTTPException(status_code=404, detail="Unbekannter Slot")
    logo_file = slot_logo_file(source_cfg, slot_id)
...
def xmltv_programme_title_and_stop(
    event: dict[str, Any],
    cfg: dict[str, Any],
    now: datetime,
) -> tuple[str, datetime]:
    ...
    display_stop = epg_stop(start, event.get("stop"), cfg, now.tzinfo)
    title = str(event.get("title") or "Event")
    try:
        technical_stop = datetime.fromisoformat(str(event["stop"]))
        if technical_stop.tzinfo is not None and now.tzinfo is not None:
            technical_stop = technical_stop.astimezone(now.tzinfo)
    except (KeyError, TypeError, ValueError):
        technical_stop = display_stop
    if start <= now < technical_stop and display_stop <= now:
        return f"Weiter verfügbar: {title}", technical_stop
    if start <= now < display_stop:
        return f"Live: {title}", display_stop
    return title, display_stop
...
def render_xmltv(state: dict[str, Any], cfg: dict[str, Any]) -> str:
    ...
    for slot in state["slots"]:
        for event in slot.get("events", []):
            source_cfg = source_config_for_key(
                cfg, str(event.get("source_key") or slot.get("source_key", "default"))
            ) or cfg
            title, stop = xmltv_programme_title_and_stop(event, source_cfg, now)
            output.append(
            ...
            output.append(
                f'    <title lang="{language}">{xml_text(title)}</title>'
            )
            if event.get("desc"):
                output.append(
...
def write_xmltv_file(state: dict[str, Any], cfg: dict[str, Any]) -> None:
    ...
    configured_socket = (
        str(cfg.get("xmltv_socket_path") or "").strip()
        if config_bool(cfg, "xmltv_socket_enabled")
        else ""
    )
    if not configured_path and not configured_socket:
        return
    ...
    if configured_path:
        ...
        try:
            ...
            LOG.exception(
                "XMLTV-Datei konnte nicht nach %s geschrieben werden", output_path
            )
            try:
    ...
    if configured_socket:
        send_xmltv_to_socket(rendered, cfg)


def send_xmltv_to_socket(rendered: bytes, cfg: dict[str, Any]) -> None:
    socket_path = str(cfg.get("xmltv_socket_path") or "").strip()
    if not socket_path:
        return
    if not hasattr(socket, "AF_UNIX"):
        LOG.error("XMLTV-Socket wird auf dieser Plattform nicht unterstützt: %s", socket_path)
        return
    try:
        timeout = float(cfg.get("xmltv_socket_timeout_seconds", 10))
    except (TypeError, ValueError):
        timeout = 10.0
    try:
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as client:
            client.settimeout(max(timeout, 0.1))
            client.connect(socket_path)
            client.sendall(rendered)
            try:
                client.shutdown(socket.SHUT_WR)
            except OSError:
                pass
        LOG.info("XMLTV an Socket gesendet: %s", socket_path)
    except OSError:
        LOG.exception("XMLTV konnte nicht an Socket %s gesendet werden", socket_path)
