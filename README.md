...
- mitgelieferte transparente Senderlogos für Amazon-, DAZN- und RTL-Event-Slots
...
| `live_countdown_enabled` | deutet `Noch X Std. Y Min.` hinter einem Live-Marker als zukünftige Startzeit |
...
Bei Quellen mit `live_countdown_enabled: true` wird ein relativer Countdown nur
beim ersten Auftauchen eines Events in eine Startzeit umgerechnet. Solange
`tvg-id` und bereinigter Titel gleich bleiben, wird diese gespeicherte Startzeit
bei weiteren Refreshes beibehalten. So verschiebt ein veralteter Provider-Text
wie `Noch 14 Std. 3 Min.` das EPG nicht fortlaufend nach hinten.
...
ob die Event-Titel ein unterstütztes Datumspräfix enthalten.
